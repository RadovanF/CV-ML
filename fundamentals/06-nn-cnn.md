...
Záporné hodnoty nás prostě nezajímají: reprezentují "neuron, který na daný vstup nereaguje". ReLU je oblíbená proto, že je výpočetně velmi jednoduchá a trénování s ní funguje spolehlivě v praxi.
...
class CNN(nn.Module):
    ...
    def forward(self, x):
        x = self.features(x)       # → (batch, 64, 5, 5)
